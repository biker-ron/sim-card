...
class USIM(UICC):
    ...
    def get_acc(self):
        ...
        reads ACC value at address [0x6F, 0x078]
        returns IMSI string on success or None on error
        '''
        # select ACC file
    ...
    def get_spdi(self):
        '''
        get_spdi() -> string(SPDI)

        reads SPDI value at address [0x6F, 0xCD]
        returns SPDI string on success or None on error
        '''
        # select SPDI file
        print ("=====================================================")
        print ("=    Get SPDI Value Using Select and GetResponse    =")
        print ("=====================================================")
        spdi = self.select( [0x3F, 0x00, 0x7F, 0x20, 0x6F, 0xCD], type="pmf" )
        if spdi is None:
            log(3, '(get_spdi) %s' % self.coms())
            return None
        # and parse the received data into the SPDI structure
        if 'Data' in spdi.keys():
            print ("==== GetResponse SPDI Values Returned: ", spdi)
            return (spdi['Data'])

        # if issue with the content of the DF_SPDI file
        if self.dbg >= 2:
            log(3, '(get_spdi) %s' % self.coms())
        return None


    def get_spdi_readBinary(self):
        '''
        get_spdi() -> string(SPDI Values)
        ...
        # select SPDI file
        print ("=====================================================")
        print ("=       Get SPDI Value Using READ BINARY            =")
        print ("=====================================================")
        self.SELECT_FILE(0, 4, [0x3F, 0x00])
        self.SELECT_FILE(0, 4, [0x7F, 0x20])
        self.SELECT_FILE(0, 4, [0x6F, 0xCD])
        spdi2 = self.READ_BINARY( Le=0x07 )
        print ("==== SPDI from binary read: ", spdi2)
        if spdi2 is None:
            log(3, '(get_spdi_readBinary) %s' % self.coms())
            return None
        else:
            return spdi2
    ...
    def get_plmnsel(self):
        '''
        get_plmnsel() -> string(PLMNSel)
        reads PLMNSel value at address [0x6F, 0x30]
        returns PLMNSel string on success or None on error
        '''
        # select PLMNSel file
        # print ("=====================================================")
        # print ("=    Get PLMN Sel Value Using Select and GetResponse    =")
        # print ("=====================================================")
        plmnsel = self.select([0x3F, 0x00, 0x7F, 0x20, 0x6F, 0x30], type="pmf")
        if plmnsel is None:
            log(3, '(get_plmnsel) %s' % self.coms())
            return None
        # and parse the received data into the SPDI structure
        if 'Data' in plmnsel.keys():
            # print ("==== GetResponse PLMN Sel Values Returned: ", plmnsel)
            return (plmnsel['Data'])
    ...
    def explore_fs(self, filename='~/usim_fs_data.txt', depth=2):
        '''
